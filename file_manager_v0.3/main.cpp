...
void ffClickedorPressedF(GtkWidget *w)
{
        if (ffClicked && !ffHold)
        {
                ffButtonFunc(w);
...
gboolean ffPressedF(GtkWidget *w)
{
        if (ffClicked)
                ffClicked = false;
        if (!ffClicked)
...
int main(int argc, char *argv[])
{
        ...
        gtk_window_set_title(GTK_WINDOW(win), "File Explorer 0.3");
        currentPath = "/'home'/'" + username + "'/";
        ffpaneInsert(runcomm("ls /home/" + username));
        gtk_main();
